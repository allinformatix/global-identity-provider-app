    ...
    return BackbonePaginator.extend({
        ...
        */
        ...
        parseState: function parseState(resp) {
            ...
            } else if (resp.totalPagedResults === -1) {
                // for JDBC (not DS), pagedResultsCookie is not empty , then there are more results
                if (resp.pagedResultsCookie === null) {
                    this.state.pagedResultsCookie = null;
                    this.state.totalPages = this.state.currentPage;
                } else {
                    this.state.pagedResultsCookie = resp.pagedResultsCookie;
                    this.state.totalPages = this.state.currentPage === 0 ? 2 : this.state.totalPages + 1;
                    this.state.totalRecords = this.state.totalPages * resp.resultCount;
                }
            } else {
