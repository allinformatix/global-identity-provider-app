...
if [[ -z "$1" ]]; then
   ...
   if [[ -f /opt/amster/create_idm_to_am_admin_user.py ]]; then
      echo "[INFO] Executing Python-based user creation"
      python3 /opt/amster/create_idm_to_am_admin_user.py
      if [[ -f /opt/amster/test_idm_admin_user.py ]]; then
         echo "[INFO] Performing Tests!"
         python3 /opt/amster/test_idm_admin_user.py
      fi
   else
      echo "[WARN] Python script not found: /opt/amster/create_idm_to_am_admin_user.py"
   fi
else
