    ...
    var RelationshipArrayView = AbstractView.extend({
        ...
        removeRelationships: function removeRelationships(e) {
            ...
            uiUtils.confirmDialog(confirmText, "danger", _.bind(function () {
                ...
                */
                _.forEach(this.data.selectedItems, _.bind(function (relationship) {
                    if (!promise) {
        ...
        getCols: function getCols() {
            ...
            _.forEach(relationshipProp.properties._refProperties.properties, _.bind(function (col, colName) {
                if (colName !== "_id") {
                    cols.push({
        ...
        deleteRelationship: function deleteRelationship(value) {
            var patchDefinition = [{
                ...
                value: _.pick(value, "_ref", "_refProperties", "_refResourceCollection", "_refResourceId")
            }];

            return this.patchRelationship(patchDefinition);
        },
        ...
        updateRelationship: function updateRelationship(value, oldVal) {
        ...
        patchRelationship: function patchRelationship(patchDefinition) {
            var patchUrl = constants.context + "/",
                patchUrlArray = this.data.prop.relationshipUrl.split("/");
            // get rid of the last element of patchUrlArray
            patchUrlArray.pop();

            return resourceDelegate.serviceCall({
                serviceUrl: patchUrl,
                url: patchUrlArray.join("/"),
                type: "PATCH",
                ...
                error: function error(e) {
                    if (e.status === 400 && e.responseJSON.message.indexOf("not null") > -1) {
                        ...
                        var resourceRefArray = patchDefinition.value._ref.split("/"),
                        ...
                        resourceDelegate.readResource(serviceUrl, resourceId).then(function (resource) {
                            var displayObject = resourceCollectionUtils.buildResourceDisplayObject(self.schema, self.data.prop, patchDefinition.value, resource);
        ...
        loadChart: function loadChart(models) {
            ...
            _.forEach(models, _.bind(function (model) {
