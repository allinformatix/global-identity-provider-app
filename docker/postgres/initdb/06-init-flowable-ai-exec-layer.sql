-- https://github.com/flowable/flowable-engine/blob/flowable-6.8.0/distro/sql/create/all/flowable.postgres.all.create.sql

-- Erstelle die Datenbank nur, wenn sie noch nicht existiert
CREATE DATABASE ai_exec_layer_workflow;

CREATE USER ai_exec_layer_workflow_user WITH PASSWORD 'changeme';
-- Optional Rechte zuweisen:
GRANT ALL PRIVILEGES ON DATABASE ai_exec_layer_workflow TO ai_exec_layer_workflow_user;

-- Verbinde dich zur Datenbank
\connect ai_exec_layer_workflow
...
BEGIN
    ...
    LOOP
        EXECUTE format(
            'ALTER TABLE public.%I OWNER TO ai_exec_layer_workflow_user;',
            tbl.tablename
        );
    END LOOP;
END $$;
...
BEGIN
    ...
    LOOP
        EXECUTE format(
            'ALTER SEQUENCE public.%I OWNER TO ai_exec_layer_workflow_user;',
            seq.sequencename
        );
    END LOOP;
END $$;
...
BEGIN
    ...
    LOOP
        EXECUTE format(
            'ALTER VIEW public.%I OWNER TO ai_exec_layer_workflow_user;',
            v.table_name
        );
    END LOOP;
END $$;
