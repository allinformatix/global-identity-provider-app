...
CREATE DATABASE openidm;

CREATE USER openidm_repo_user WITH PASSWORD 'changeme';
-- Optional Rechte zuweisen:
GRANT ALL PRIVILEGES ON DATABASE openidm TO openidm_repo_user;

-- Verbinde dich zur Datenbank
\connect openidm

DROP SCHEMA IF EXISTS openidm CASCADE;
CREATE SCHEMA openidm AUTHORIZATION openidm_repo_user;
...
BEGIN
    FOR tbl IN
        SELECT tablename
        FROM pg_tables
        WHERE schemaname = 'openidm'
    LOOP
        EXECUTE format(
            'ALTER TABLE openidm.%I OWNER TO openidm_repo_user;',
            tbl.tablename
        );
    END LOOP;
END $$;
...
BEGIN
    ...
    LOOP
        EXECUTE format(
            'ALTER SEQUENCE openidm.%I OWNER TO openidm_repo_user;',
            seq.sequencename
        );
    END LOOP;
END $$;
...
BEGIN
    ...
    LOOP
        EXECUTE format(
            'ALTER VIEW openidm.%I OWNER TO openidm_repo_user;',
            v.table_name
        );
    END LOOP;
END $$;
