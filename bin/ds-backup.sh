...
HOSTS="ds-idrepo-0,ds-cts-0"

### IDREPO SCHEDULE ###
# Cron schedule for backup task to run
BACKUP_SCHEDULE_IDREPO="*/30 * * * *"

### Dynamische STAGE-basierte Konfiguration ###
STAGE=${STAGE:-dev}  # Falls STAGE nicht gesetzt ist, Standard: dev

case "$STAGE" in
  dev)
    BACKUP_DIRECTORY_IDREPO="s3://dev-gidp-backups"
    BACKUP_DIRECTORY_CTS="s3://dev-gidp-backups"
    ;;
  stg)
    BACKUP_DIRECTORY_IDREPO="s3://stg-gidp-backups"
    BACKUP_DIRECTORY_CTS="s3://stg-gidp-backups"
    ;;
  prd)
    BACKUP_DIRECTORY_IDREPO="s3://prd-gidp-backups"
    BACKUP_DIRECTORY_CTS="s3://prd-gidp-backups"
    ;;
  *)
    echo "❌ Ungültiger STAGE-Wert: $STAGE"
    exit 1
    ;;
esac
...
# BACKUP_DIRECTORY_IDREPO="s3://dev-gidp-backups"
# Backends to backup.
BACKENDS_IDREPO="amIdentityStore,cfgStore,idmRepo,gidpOrgDataRoot,gidpConsumerTenantsRoot,gidpBusinessTenantsRoot"
